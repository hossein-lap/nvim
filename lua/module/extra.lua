...
function AutoFillEnable(notify_me)
	...
	if notify_me then
		vim.notify("Autofill is enabled", 2,
		           {title = 'extra.lua'})
	end
end
...
function AutoFillDisable(notify_me)
	...
	if notify_me then
		vim.notify("Autofill is disabled", 2,
		           {title = 'extra.lua'})
	end
end
--}}}
...
function AutoFillMarkdownEnable(notify_me)
	...
	if ft == 'markdown' or ft == 'rmd' then
		...
		if notify_me then
			vim.notify("Markdown Autofill is enabled", 2,
			           {title = 'extra.lua'})
		end
	end
end
...
function AutoFillMarkdownDisable(notify_me)
	...
	if ft == 'markdown' or ft == 'rmd' then
		...
		if notify_me then
			vim.notify("Markdown Autofill is disabled", 2,
			           {title = 'extra.lua'})
		end
	end
end
...
function AutoFillSentEnable(notify_me)
	...
	if ft == 'sent' or ft == 'text' then
		...
		if notify_me then
			vim.notify("Sent Autofill is enabled", 2,
			           {title = 'extra.lua'})
		end
	end
end
...
function AutoFillSentDisable(notify_me)
	...
	if ft == 'sent' or ft == 'text' then
		...
		if notify_me then
			vim.notify("Sent Autofill is disabled", 2,
			           {title = 'extra.lua'})
		end
	end
end
