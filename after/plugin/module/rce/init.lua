...
-- _run {{{
function _run(wrapped, work)
	work = work or 'wrap'
	local style = {
		vertical = 'vs ',
		horizontal = 'split ',
	}
	local buffercmd = style[TermWindowStyle] or 'float'
...
-- }}}
-- _quickfix {{{
function _quickfix(command, opts)
	if opts == nil then
		opts = {}
	end
	...
	if command == nil then
		vim.notify('_quickfix() at least needs one <command> to execute',
			        3, {title = '_quickfix()'})
		return 294
	end
	...
	vim.notify(string.format('[%s]: %s', prompt, command), 2)
...
function wrapcmd(ft, exe)
	...
	if work == 'run' then
		...
		_quickfix(command, {prompt = work, silent = false})
	end
end
-- }}}
...
vim.keymap.set('n', '<localleader>fe',
	        ':lua wrapcmd(vim.bo.filetype, "run")<CR>',
	        { silent = true, desc = 'Run current file' })
vim.keymap.set('n', '<localleader>fw',
	        ':lua wrapcmd(vim.bo.filetype, "compile")<CR>',
	        { silent = true, desc = 'quickfix › Compile current file' })
vim.keymap.set('n', '<localleader>fq',
	        ':lua wrapcmd(vim.bo.filetype, "debug")<CR>',
	        { silent = true, desc = 'quickfix › Debug current file' })

---- makefile
vim.keymap.set('n', '<localleader>ca', ':lua _run("make")<CR>',
	        { silent = true, desc = 'make › all' })
vim.keymap.set('n', '<localleader>cc', ':lua _quickfix("make", {silent = true})<CR>',
	        { silent = true, desc = 'quickfix › make' })
vim.keymap.set('n', '<localleader>cd', ':lua _quickfix("make clean", {silent = true})<CR>',
	        { silent = true, desc = 'quickfix › make clean' })
vim.keymap.set('n', '<localleader>cf', ':lua _quickfix("make force", {silent = true})<CR>',
	        { silent = true, desc = 'quickfix › make force' })
vim.keymap.set('n', '<localleader>cb', ':lua _quickfix("make build", {silent = true})<CR>',
	        { silent = true, desc = 'quickfix › make build' })
vim.keymap.set('n', '<localleader>cF', ':lua _quickfix("make full", {silent = true})<CR>',
